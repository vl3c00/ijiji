...
// ✅ Fixed: Explicitly defined `years` as number[]
function YearSelector({ period, setPeriod, years }: { 
  period: Period; 
  setPeriod: (period: Period) => void; 
  years: number[]; // Ensure `years` is an array of numbers
}) {
  return (
    <Select
    ...
    >
      ...
      <SelectContent>
        {years.map((year: number) => ( // ✅ Explicitly type `year` as number
...
// ✅ Fixed: Removed `years` prop (was mistakenly required)
function MonthSelector({ period, setPeriod }: { 
  ...
  return (
    <Select
      ...
      onValueChange={(value) => {
        setPeriod({
          year: period.year, // ✅ Fixed: Keep `year` unchanged
          month: parseInt(value), // ✅ Fixed: Correctly update `month`
        });
      }}
    >
      <SelectTrigger className="w-[180px]">
