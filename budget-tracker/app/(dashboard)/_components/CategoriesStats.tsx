...
function CategoriesCard({
  ...
  // Fixing the TypeScript implicit `any` error
  const filteredData = data.filter((el: GetCategoriesStatsResponseType[number]) => el.type === type);

  const total = filteredData.reduce(
    (acc: number, el: GetCategoriesStatsResponseType[number]) => acc + (el._sum?.amount || 0),
    0
  );
        ...
        {filteredData.length > 0 && (
          <ScrollArea className="h-60 w-full px-4">
            <div className="flex w-full flex-col gap-4 p-4">
              {filteredData.map((item: GetCategoriesStatsResponseType[number]) => {
