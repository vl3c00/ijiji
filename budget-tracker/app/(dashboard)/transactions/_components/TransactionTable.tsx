...
function TransactionTable({from, to}: Props) {
    ...
    const handleExportCSV = (data: TransactionHistoryRow[]) => {
        const formattedData = data.map(({ date, createdAt, updatedAt, ...rest }) => ({
            ...rest,
  ...
  onClick={() => {
    const data = table.getFilteredRowModel().rows.map(row => ({
      id: row.original.id,
      createdAt: new Date(row.original.createdAt),  // Ensure it's a Date object
      ...
      date: new Date(row.original.date),  // Ensure it's a Date object
    }));
