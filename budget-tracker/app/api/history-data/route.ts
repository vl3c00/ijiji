...
async function getYearHistoryData(userId: string, year: number) {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        // Find the month data and ensure null is handled
        const monthData = result.find((row) => row.month === i);
        if (monthData) {
            expense = monthData._sum.expense ?? 0; // Handle null values safely
            income = monthData._sum.income ?? 0;  // Handle null values safely
        }
...
async function getMonthHistoryData(userId: string, year: number, month: number) {
    ...
    for (let i = 1; i <= daysInMonth; i++) {  // Loop only up to the number of days in the month
        let expense = 0;
        let income = 0;

        // Find the day data and ensure null is handled
        const dayData = result.find((row) => row.day === i);
        if (dayData) {
            expense = dayData._sum.expense ?? 0;  // Handle null values safely
            income = dayData._sum.income ?? 0;   // Handle null values safely
        }
