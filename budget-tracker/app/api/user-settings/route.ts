...
import { currentUser } from "@clerk/nextjs/server";  // Use getAuth from @clerk/nextjs/server
import { revalidatePath } from "next/cache";
import { redirect } from "next/navigation";

export async function GET() {
  // Use getAuth to retrieve the current user from the request
  const user = await currentUser();  // Correctly access the user from the request
